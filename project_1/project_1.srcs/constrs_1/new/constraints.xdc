...
#define RESET_VALUE	        (2^32) - 12500// maximum count F (1.000.000.000)
...
int main(void){
	...
	SongSize = SdStartSong("BATTLE.wav");
...
void TimerCounterHandler(void *CallBackRef, u8 TmrCtrNumber)
{
	...
	if (XTmrCtr_IsExpired(&TimerCounterInst,0)){
		...
		leds = ~leds;
		SongSample = PlaySong();
		XGpio_DiscreteWrite(&GPIO, 2, SongSample);
...
u16 PlaySong()
{
	...
	CurrentByte += 2;
	sample  = (ReadBuffer[0]) | (ReadBuffer[1] << 8);
	pos_sample = abs(sample);
	if(sample <0){
		pos_sample =  32768 - pos_sample;
	}
	else{
		pos_sample =  32768 + pos_sample;
	}
	_12bit_sample = pos_sample & 0xFFF0 >> 4;
//	pos_sample = sample + 32768;
//	xil_printf("muestra es: %d\n\r", pos_sample);
	return _12bit_sample;
