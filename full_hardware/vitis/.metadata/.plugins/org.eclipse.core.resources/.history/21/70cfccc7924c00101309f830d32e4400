...
//	LCD_Clear(GUI_BACKGROUND);
	...
	Open_Sprite("0:/pokemon.BMP", &pok_sprite);


	AudioSize = SdStartSong("0:/BATTLE.wav", &Rfp, &CurrentByte, &AudioPlayerState); //Apertura de archivo de audio
...
void AudioTimerHandler(void *CallBackRef)
{
	...
	if (XTmrCtr_IsExpired(&AudioTimerInst, TIMER_CNTR_AUDIO)){
	    ...
	    if(AudioPlayerState == 0){
	    	...
	    	if(CurrentByte + 4 >= AudioSize + 44){
	    		AudioSize = SdResumeSong("BATTLE.wav", &Rfp, &CurrentByte, &AudioPlayerState);
