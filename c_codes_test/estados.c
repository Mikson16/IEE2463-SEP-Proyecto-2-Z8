...
void imprimir_active_trainers(State *state)
{
    ...
    {
        printf("Trainer 1 activo: %d\n", state->trainer_1->active);
        printf("Trainer 2 inactivo: %d\n", state->trainer_2->active);
    }
    else if (state->trainer_2->active == 1 && state->trainer_1->active == 0)
    {
        printf("Trainer 1 inactivo: %d\n", state->trainer_1->active);
        printf("Trainer 2 activo: %d\n", state->trainer_2->active);
    }
    else
    {
...
State *state_init(State_setup *setup)
{
    ...
    printf("Estado final creado con id: %d\n", state_final->state_id);
...
State *state_main(State *state)
{
    ...
    int opcion;
    printf("[MAIN] Elige una opción:\n");
    printf("1. Ir a SELECTION\n");
    printf("2. Ir a BATTLE\n");
    scanf("%d", &opcion);
    if (opcion == 1)
    {
    ...
    {
        printf("[MAIN] Opción no válida. Permaneciendo en el estado principal.\n");
...
State *state_selection(State *state)
{
    ...
    int opcion;
    printf("[SELECTION] Elige una opción:\n");
    printf("1. Ir a INFO\n");
    printf("2. Volver a MAIN\n");
    scanf("%d", &opcion);
    if (opcion == 1)
    {
    ...
    {
        printf("[SELECTION] Opción no válida. Permaneciendo en el estado de selección.\n");
...
State *state_info(State *state)
{
    ...
    // Por ahora, solo imprimimos el id del estado
    printf("[INFO] con id: %d\n", state->state_id);
    imprimir_active_trainers(state); // Imprime los trainers activos en el estado actual
    // Aquí podrías agregar más lógica relacionada con el estado de información
    /*Logica basica para cambiar de estado por meido de input de consola*/
    int opcion;
    printf("[INFO] Elige una opción:\n");
    printf("1. Volver a SELECTION\n");
    scanf("%d", &opcion);
    if (opcion == 1)
    {
    ...
    {
        printf("[INFO] Opción no válida. Permaneciendo en el estado de información.\n");
...
State *state_battle(State *state)
{
    ...
    // Por ahora, solo imprimimos el id del estado
    printf("[BATTLE] Estado de batalla con id: %d\n", state->state_id);
    imprimir_active_trainers(state); // Imprime los trainers activos en el estado actual
...
State *state_damage(State *state)
{
    ...
    // Por ahora, solo imprimimos el id del estado
    printf("[DAMAGE] Estado de daño con id: %d\n", state->state_id);
    imprimir_active_trainers(state); // Imprime los trainers activos en el estado actual
    ...
    printf("[DAMAGE] Calculando daño...\n");
    printf("[DAMAGE] Daño calculado. Avanzando al estado final...\n");
...
State *state_final(State *state)
{
    ...
    // Por ahora, solo imprimimos el id del estado
    printf("[FINAL] Estado final con id: %d\n", state->state_id);
    imprimir_active_trainers(state); // Imprime los trainers activos en el estado actual
    // Aquí agregar más lógica relacionada con el estado final

    /*Test para simular que el juego continua o termina*/
    int opcion;
    printf("[FINAL] Elige una opción:\n");
    printf("1. Terminar el juego\n");
    printf("2. Volver al estado principal\n");
    scanf("%d", &opcion);
    if (opcion == 1)
    {
        printf("[FINAL] Terminando el juego...\n");
        return NULL; // Retorno null para cerrar el flujo de estados
    }
    else if (opcion == 2)
    {
        printf("[FINAL] Volviendo al estado principal...\n");
        if (state->trainer_1->active == 1 && state->trainer_2->active == 0)
        {
        ...
        {
            printf("[ERROR FINAL] Error al cambiar de trainer activo.\n");
        }
...
{
    printf("Flujo de estados iniciado...\n");
    ...
    {
        printf("Estado desconocido con id: %d\n", state->state_id);
