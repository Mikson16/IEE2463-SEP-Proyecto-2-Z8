...
#define RESET_VALUE	        (2^32) - 5000000// 20 Hz
        ...
        u16      DeviceId)
{
	XScuGic_Config *IntcConfig;
	    ...
	    XTmrCtr_SetHandler(&TimerInst0, (XTmrCtr_Handler)readInputs, &TimerInst0);
	  	XTmrCtr_SetResetValue(&TimerInst0, TIMER_CNTR_0, RESET_VALUE);
	  	XTmrCtr_SetOptions(&TimerInst0, TIMER_CNTR_0, XTC_INT_MODE_OPTION | XTC_AUTO_RELOAD_OPTION);
...
prev_bitmap = new_bitmap;
	...
	xil_printf("Luz :%i\n", read_opt());
	xil_printf("\n");
	XTmrCtr_Reset(&TimerInst0, 0);
	XTmrCtr_Start(&TimerInst0, 0);
}
}
void lightHandler(void *CallBackRef)
{
	XGpio_InterruptDisable(&gpio1, LIGHT_INT);
	float light;
	light = read_opt();
	if(light < 10){
		text_color = CYAN;
	...
	(void)XGpio_InterruptClear(&gpio1, LIGHT_INT);
	XGpio_InterruptEnable(&gpio1, LIGHT_INT);
